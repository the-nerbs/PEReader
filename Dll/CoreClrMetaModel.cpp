// see: 
// https://github.com/dotnet/coreclr/blob/a9b25d4aa22a1f4ad5f323f6c826e318f5a720fe/src/md/runtime/metamodel.cpp
//
// This file has adaptations to generate the <Table> elements of the metadata schema.
//
// When updates are needed, just preprocess this to a file.
...
#define SCHEMA_TABLE_START(tbl)         <Table name=STRING(tbl)>
#define SCHEMA_ITEM_NOFIXED()             <NoFixed />
#define SCHEMA_ITEM(tbl,typ,col)          <Item type=STRING(typ) name=STRING(col) />
#define SCHEMA_ITEM_RID(tbl,col,tbl2)     <RidItem name=STRING(col) table=STRING(tbl2) />
#define SCHEMA_ITEM_STRING(tbl,col)       <StringItem name=STRING(col) />
#define SCHEMA_ITEM_GUID(tbl,col)         <GuidItem name=STRING(col) />
#define SCHEMA_ITEM_BLOB(tbl,col)         <BlobItem name=STRING(col) />
#define SCHEMA_ITEM_CDTKN(tbl,col,tkns)   <CodedTokenItem name=STRING(col) tokens=STRING(tkns) />
#define SCHEMA_TABLE_END(tbl)           </Table>
